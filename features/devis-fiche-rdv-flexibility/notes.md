...
- **C4b** `DentalRecordDto.TreatmentPlanItemIds` (every act of the devis the fiche carries); the patient page adds each to the modal's plan options, and `markCarriedOnPlan` marks those cards « sur le devis » (no price field). With a6's implicit « Ajouter au devis » they read as acts being ADDED, with an editable price the server then dropped. ⚠️ `planActKey` (the card the step line and « L'acte entier est chiffré … » attach to) picks the LEAD act's card — « the first carried card » was only true while one act could be carried, and put a couronne's « étape 2 sur 2 » and 300 DT on a détartrage.
