...
- **`Common/ClinicClock.cs`** — the clinic's wall clock (Tunisia **UTC+1**, no DST). Replaces two byte-identical private copies of `ResolveTunisiaTimeZone()` (A-21). `ToClinicLocal`/`ToUtc`/`ClinicToday`/`ClinicYear`, plus `StartOfLocalDayUtc`/`EndOfLocalDayUtc` which return an **explicit UTC instant** — `ApplicationDbContext` treats `Unspecified` as UTC on write, so handing a bare local value to a query would shift every boundary by an hour.
