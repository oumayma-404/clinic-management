...
Infrastructure layer (Clean Architecture). Implements the persistence + external-integration concerns declared as interfaces in the Domain and Application layers: EF Core/PostgreSQL data access, repository implementations, file storage (MinIO + local), Google Calendar two-way sync, AI services (HuggingFace, Google AI, AI action dispatch), notifications, PDF generation, Auth0 management, and — for Local/offline installs — database backup (`pg_dump`) and self-generated HTTPS trust material. All wiring lives in `Extensions.cs` (`AddInfrastructure`).
...
### Backup (Phase 5, Local mode) — `Services/`
- **`PgDumpBackupService`** (`IBackupService`) — one-click "Backup now" (US-8 / FR-G). Dumps PostgreSQL with the bundled `pg_dump.exe` (custom format `-Fc`, `pg_restore`-able) then recursively copies the file-storage folder, both into a **unique** timestamped `clinic-backup-<yyyyMMdd-HHmmss>[-N]` subfolder (DB first, R-3). The codebase's **only** `Process` shell-out: launched with an argument **list** (no shell string → no injection), DB password via the `PGPASSWORD` env var (never on the command line / in logs), bounded by a configurable timeout that kills the process tree. Fails **loud** — missing `pg_dump`, unwritable destination, insufficient free space (pre-check factors in the live DB size via `pg_database_size`), or a non-zero `pg_dump` exit each throw `InvalidOperationException` with a clear French operator message; a partial backup folder is deleted before rethrow so a half-written backup is never restorable-looking.

### Other
- **`PdfGenerationService`** (`IPdfGenerationService`) — QuestPDF (Community license). Renders French medical documents: `prescription` (ORDONNANCE), `liaison`, `honoraires`, `certificat`. Parses `MedicalDocumentPdfData.Content` (handles both JSON-array and legacy string formats). **`bulletin-cnam` is the exception**: it branches out of QuestPDF to the BS1 overlay renderer (below) — the old custom bulletin-cnam QuestPDF table was removed.
- **`CnamBs1BulletinRenderer`** (`Services/`, internal) — stamps `bulletin-cnam` data onto the genuine CNAM **BS1** form (`Assets/BS1.pdf`, bundled + copied to output) at coordinates calibrated to the 2-page A4-landscape form, using **PdfSharp** (MIT). Fills only the dentist-relevant regions (Identifiant Unique digit comb, régime/lien ticks, assuré + malade identity, the "Consultations et actes de soins dentaires" 6-row table incl. per-row `Doctor.CodeProfessionnelSante`, and the page-2 "Cadre de soins" APCI/MO/Hospitalisation/Suivi ticks + code APCI); everything else stays blank (physical cachet/signature). Overflow >6 acts appends copies of the identity+acts page so no act is dropped; a missing/unreadable BS1 asset **fails fast** with a French operator message (never a blank/fallback PDF). Data comes entirely from `MedicalDocumentPdfData.Content` (the download path receives it straight from the frontend editor; the `PdfGenerationJob` path rebuilds it from the persisted `ContentJson`).
- **`Bs1FontResolver`** (`Services/`, internal, `PdfSharp.Fonts.IFontResolver`) — process-wide font resolver the BS1 renderer installs idempotently before first render (core PdfSharp ships no fonts). Maps all text to one sans-serif face loaded from the OS (Windows Arial first, then Liberation/DejaVu on Linux); fails fast in French if none is found.
...
### LAN hosting / security helpers (root namespace) — Phases 4–5, Local mode
...
- **`LocalInstallPaths`** (Phase 5, R-6) — resolves per-install paths against the **install directory** (`AppContext.BaseDirectory`), not the CWD: a Windows service's CWD is `System32`, where relative `.local/`, `Files/`, `logs/` would land wrong. `Resolve(pathOrRelative)` (absolute passes through), `LocalDir` (the gitignored `.local/`), `LocalFile(name)`. Only Local paths use it → Cloud unchanged.

### Certificate provisioning (`Security/`) — Phase 5 S3, Local mode
- **`CertificateProvisioner`** — self-generates the LAN HTTPS trust material on first Local boot (FR-E2): a self-signed **CA** (`CN=Clinic Management Local CA`, 10y) and a **server leaf** it signs (5y), whose SANs cover the machine hostname, `localhost`, `127.0.0.1`, and every non-loopback IPv4 so LAN clients connect by IP or name (AC-2.5). `EnsureServerCertificate()` is **idempotent** — an existing, loadable set (proven by opening the PFX with its stored password) is reused so the CA the clients trust is stable across restarts. Writes `.local/server.pfx` (random per-install password in `.local/server-cert-password`) for Kestrel and exports the public CA to `.local/ca.crt` for the client installer to import into the Windows Root store (S7). Constructed **manually pre-`builder.Build()`** in `Program.cs` (Kestrel needs the cert before the DI container exists) — **not** DI-registered.
