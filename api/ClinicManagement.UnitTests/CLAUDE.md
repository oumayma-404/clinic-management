...
- **`Common/Behaviors/RealtimeResourceResolverTests.cs`** — the backend↔frontend realtime key contract, and a worked example of a guard that derives both sides instead of listing them (AC-P4.23–4.25). It reflects over every `IBaseRequest` in the Application assembly for the emitted set, **parses `web/lib/realtime/clinic-hub.ts`** for the declared set, and asserts they are **equal in both directions**; two allow-lists (emit-only / listen-only) exist for intentional asymmetry and are asserted **empty**. It replaced a 16-row `[InlineData]` table that stayed green throughout the entire period five keys were broadcast with nothing listening (audit § 9.1) — a table can only fail on rows someone remembered to write, never on the new area. It finds the frontend file via **`[CallerFilePath]`**, not `AppContext.BaseDirectory`, because the suite is routinely built to a scratch OutDir outside the repo (the SAC workaround); and it **throws** rather than skipping when the file is absent. ⚠️ Since `large-file-transfer` Part 2 it also holds **`IDoesNotBroadcast`**, the per-command veto on a broadcast the *area* would otherwise emit: sending one file in four parts made every browser in the clinic refetch the patient's drawer four times before the file existed, because each accepted chunk looked like an edit to `Files`. Three cases pin the steps emitting nothing and — the direction silence would break — the **completion still emitting `files`**. The fourth is derived over the whole assembly and catches a marker that changes *nothing*, i.e. one worn in an already-excluded area, since a redundant marker reads to the next author as a live one; proven red by marking `LoginCommand`.
...
- **`Domain/AuditChainTests.cs`** + **`Common/LogTemplateCoverageTests.cs`** + **`Common/ContentSecurityPolicyAgreementTests.cs`** (`hosted-security-hardening` Part D). The chain class is the highest-value one: every audit entry's tamper-evidence rests on it, and a wrong hash is wrong everywhere at once and visible nowhere. ⚠️ Its load-bearing case is `The_Hash_Survives_The_Round_Trip_Through_The_Columns_Own_Precision` — PostgreSQL's `timestamptz` holds **microseconds** while a .NET `DateTime` holds 100 ns ticks, so without the canonical form every single entry would report as altered, on a check whose whole value is being believed when it says so. Its siblings pin that pre-chain history is **counted** rather than reported as tampering, while an unchained row appearing *after* a chained one is a **break** (which is what erasing a hash to hide an edit looks like), and that a declared gap is inside the chain rather than a hole in it.
  `LogTemplateCoverageTests` scans every log statement in the solution for a patient-identifying placeholder and its exemption map is **empty**; it carries both a red proof and a *masked-value* proof, since a guard that cannot distinguish `{PatientName}` from `LogMask.Name(patientName)` would force the next author to rename the field rather than mask the value. `ContentSecurityPolicyAgreementTests` parses the real `deploy/Caddyfile` (**both** sites) and the real `console/next.config.ts` and asserts byte-identity with `SecurityHeadersMiddleware.ContentSecurityPolicy` — the middleware's own docstring had said « the two must be changed together » since `multi-tenant-cloud` US-6 and **nothing enforced it**. All three find their files through `[CallerFilePath]`, not `AppContext.BaseDirectory`, for `RealtimeResourceResolverTests`' reason, and **throw** rather than skipping when a file is missing.
...
- **`Common/TreatmentStepLoadingCoverageTests.cs`** + **`Domain/Entities/TreatmentPlanItemStepTests.cs`** + **`Domain/AppointmentStepBookingTests.cs`** (`multi-seance-treatment-steps`). The coverage class is `RecoveryCodeLoadingCoverageTests`' lesson applied to the collection this feature added: `Steps` projects a private backing list and there is no lazy loading, so an unloaded navigation is **empty, not stale** — `HasSteps` reads `false` and a half-finished bridge is written straight to « réalisé », silently, and the next save persists it. ⚠️ **Its own tripwire caught a flaw in itself**: the first declaration regex used `Task<[^>]*TreatmentPlan[^>]*>`, which cannot cross a nested generic, so it saw 2 reads instead of 4 and silently exonerated both list reads — worth repeating that a derived guard needs a **non-zero candidate assertion** at every level. ⚠️ It scans **production code only**, like `ClinicCreationEntitlementTests`: a fixture builds its plan in memory with the steps attached, so admitting the test project would mean an exemption per test file. Red-proofed by removing one `ThenInclude` — it names all six real write paths. `TreatmentPlanItemStepTests` leads with the **parity** cases (an act with no steps behaves exactly as it always did), because that is the property the migration's safety rests on, and its load-bearing case is `Three_Steps_Of_One_Act_Are_Recorded_By_Three_Different_Fiches` — the refusal this feature exists to remove. `AppointmentStepBookingTests` exists for one guard: `SetProcedures` held two duplicate rules and **both** refused « préparation + empreinte dans la même séance », so most of the class is about what the relaxation must still refuse.
...
- **When a filter moves from a handler into SQL, its handler tests become vacuous rather than wrong — rewrite them,
  ...
  real defect nothing else in this project can see. See `GetMedicationsQueryHandlerTests` for the shape, and say out loud in the class docstring that the matching itself
