...
public class Clinic : AggregateRoot<Guid>
{
    ...
    // ⚠️ `GoogleCalendarHoldsOnlyAppointments` was here — the practice's declaration that its calendar held
    // nothing but appointments, which opened the Google→App import gate to events titled « Prénom Nom ». It went
    // with the import: the only thing that ever read it was the pull direction, so left standing it would have
    // been a switch in the settings that changed nothing.
