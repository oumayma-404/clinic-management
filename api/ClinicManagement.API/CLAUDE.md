...
**Billing / CNAM / clinical-depth controllers** (all `[Authorize]`, clinic-scoped) — the deepest, fully-wired subsystems:
- `InvoicesController` + `BillingController` — notes d'honoraires (invoices + lines + payments), issue/pay lifecycle, PDF, and the unified per-patient billing summary (`solde patient`).
- `TreatmentPlansController` — devis / treatment plans (planned acts + payment installments).
- `CnamNomenclatureController` + `DentalActsController` — CNAM dental nomenclature + letter-key values + dental-act catalog (backs BS1 bulletin reimbursement estimates).
- `MedicationsController` — medication catalog (backs the ordonnance picker); `OdontogramController` — per-patient tooth-state history; `DashboardController` — dashboard KPIs (incl. monthly revenue + outstanding); `DoctorsController`, `StockController`.
- El Fatoora (TTN) e-invoicing is queued off invoices and dispatched by the recurring `EInvoiceOutboxJob` (see Background Jobs).
...
|---|---|---|
| `NotificationJob.ProcessPendingNotifications()` | **SMS/WhatsApp appointment-reminder dispatcher.** Connectivity-gated (`IInternetProbe` — offline sends nothing, no retry consumed), routes each due `Pending` row to the `IReminderChannelSender` matching its channel, normalizes the patient phone to +216, commits per row, bounded retry (`RecordFailedAttempt` → `Failed` at `Reminders:MaxRetries`). `[DisableConcurrentExecution]` + `[AutomaticRetry(3)]`. | **Active** — `Cron.Minutely`. No-ops until a `Reminders` channel + credentials are configured. |
| `EInvoiceOutboxJob.DispatchQueuedInvoices()` | **TTN « El Fatoora » e-invoice outbox dispatcher.** Sends queued e-invoices when the server has internet; connectivity-gated and safe to run unconditionally (does nothing until a clinic enables e-invoicing and queues an invoice). | **Active** — `Cron.Minutely` (`"dispatch-einvoices"`). |
...
**Net effect: two active recurring jobs — the minutely `NotificationJob` reminder dispatcher and the minutely `EInvoiceOutboxJob` e-invoice dispatcher** (both connectivity-gated; the former placeholder `AISummaryJob` was removed in `reliability-and-polish`); Hangfire also serves the on-demand `PdfGenerationJob`. **Google→App calendar sync no longer has a job**: the disabled `GoogleCalendarSyncJob` class + its commented recurring registration were removed as dead scaffolding (`french-localization-and-cleanup`); `Program.cs` keeps only the defensive `RecurringJob.RemoveIfExists("sync-google-calendar")`. App→Google sync still runs inline on appointment create/update; Google→App runs only via the manual `GoogleCalendarController` endpoint.
...
On startup it applies EF migrations (FR-F3) — but **mode-branched to keep the Windows service startable**. **Cloud** runs `context.Database.Migrate()` synchronously in `Program.cs` (no service-start timeout applies), byte-for-byte. **Local** instead registers `Startup/DeferredStartupService` (an `IHostedService`, Local-only) that applies migrations **fire-and-forget after the host reports "started"**: the API runs as a Windows service there, and the SCM kills any service that doesn't reach "running" within ~30s — applying ~21 migrations to a fresh DB synchronously (plus first-boot JIT of the freshly-extracted assemblies) blew past that and the service was killed mid-migration (SCM event 7009). Deferring lets Kestrel bind → SCM sees "running" in seconds; migrations finish in the background before the first DB-touching request (first-run setup). The service's DB-unreachable handling moved with it (`StartupDiagnostics.ReportFatal` + `IHostApplicationLifetime.StopApplication()`, replacing the old `return 1`). Publish enables **`PublishReadyToRun`** so cold start is fast enough for the SCM window. After migrations comes the recurring-job (de)registration. The **outer** try/catch has a `catch … when (startupIsLocalMode && StartupDiagnostics.IsAddressInUse(ex))` → clear "port in use" message + `return 1`; else `Log.Fatal` + rethrow, with `Log.CloseAndFlush()` in `finally`.
