...
| `api/treatment-plans` | `TreatmentPlansController` | Devis / plans (planned acts + installments). GET responses carry a **derived read-back** — per-act `scheduledAppointmentId/At/Status`, plan-level `itemsDone/itemsTotal`, `nextAppointmentAt` and the linked-invoice triple — computed per request by `TreatmentPlanWorkflowProjection` (two batched reads, never per plan). `POST {id}/cancel` is `AdminOrDoctor`, pinned by `TreatmentPlansControllerAuthorizationTests`. *(Seeding a plan from the odontogram is a **frontend** prefill — there is no backend seed command.)* | `[Authorize]` (+ `AdminOrDoctor` action) |
