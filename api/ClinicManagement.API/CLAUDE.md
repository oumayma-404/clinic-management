...
**Net effect: the minutely `NotificationJob` reminder dispatcher is the one active recurring job** (`AISummaryJob`/calendar-sync stay disabled); Hangfire also serves the on-demand `PdfGenerationJob`; calendar sync runs synchronously/manually.
