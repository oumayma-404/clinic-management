...
- **Every blob knows whose it is (`multi-tenant-cloud` US-5 / Part E)**: new storage keys are
  ...
  **both** backends. The defect was not only the flat keys: « which clinic owns this blob » had **two** answers.
  Four upload sites prefixed a path of their own with a bare `{clinicId}/` (the logo, a doctor's cachet, the
  e-invoice artifacts) while four wrote `{guid}-{timestamp}` with no clinic in it at all — the patient files and
  ...
  **require** a `Guid clinicId`, so an unprefixed key is not something a caller can write, and the second
  ...
  version works for every HTTP path and fails silently for the one that matters: `EInvoiceService` uploads from
  the outbox job under **`UseSystemWide`**, where there is no clinic in scope at all.
