...
- **Other stubs/placeholders**: `NotificationService` (outbound email/SMS) logs instead of sending; `PatientSummaryService` is a string template (no AI call); `GoogleAIService` exists but isn't registered (HuggingFace is the wired AI backend).
