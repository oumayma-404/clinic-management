...
- **Real outbound SMS/WhatsApp reminders** (feature `sms-whatsapp-reminders`): the previously-dormant `Notification` outbox is now live for **SMS + WhatsApp** appointment reminders — `IReminderChannelSender` (`HttpSmsSender`/`WhatsAppSender`) + `RemindersConfig`/`ReminderSchedule`/`ReminderPhone`, enqueued best-effort post-commit by `IReminderScheduler`/`ReminderScheduler` from the appointment handlers and dispatched by the connectivity-gated minutely `NotificationJob`. Secrets come from env; nothing sends until a channel + credentials are configured.
- **Other stubs/placeholders**: `NotificationService` (outbound **email**) logs instead of sending — now superseded for SMS/WhatsApp by the reminder senders above, retained only as the dormant email path; `PatientSummaryService` is a string template (no AI call); `GoogleAIService` exists but isn't registered (HuggingFace is the wired AI backend).
- **`ValidationBehavior` is inert**: no FluentValidation validators exist; handlers validate inline and return `Result.Failure`.
- **Frontend data-wiring**: dashboard stats, `appointment-list`, the stock feature, and the notification center are all API-wired (`dashboardApi`, `useAppointments`, `stockApi`, `notificationsApi`/`useNotifications`). The old hardcoded `notifications-list` sample component still exists in the tree but is no longer mounted on the dashboard (superseded by the notification center).
