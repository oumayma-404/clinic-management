...
| `/abonnement` | `app/abonnement/page.tsx` | **« Abonnement »** (`clinic-subscription` Part C) — state card → tarif → comment payer → nous contacter, then the admin-only payment history (`components/subscription/subscription-history-table.tsx`). Single column up to **`lg:`** and two columns above it, deliberately not `md:`: a tablet portrait is 820 px and the spec asks for one readable measure there. ⚠️ **Open to every role including a secretary** — a deliberate exception to the « no clinic-wide money screen » rule, because these amounts are owed to the software *vendor* (FR-2) and reception is who meets the refused save that points here; only the **history** is admin-gated, and for a non-admin it is replaced by `AccessDeniedCard` so its fetch never fires. ⚠️ **Only an explicit 404 is read as « this deployment has no subscriptions »**; every other failure — a network drop is `ApiError(0)` — is a retryable « Réessayer » (EC-13). The authority is `authApi.getMode()`'s `requiresSubscription`, read `=== true`; a *failed* probe falls through to the read (the `/join` and `/signup` precedent). ⚠️ Until Part D the rail entry is unconditional, so on `SelfHostedLan`/`CloudBrowser` this page renders « cette installation ne fonctionne pas par abonnement » rather than nothing |
