...
export function DentalActsTable({ onEdit, onAdd, onChanged, reloadToken }: DentalActsTableProps) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      toast.success(`Acte ${quoteFr(actToDelete.codeActe)} désactivé.`)
      setActToDelete(null)
      onChanged()
    } catch (err) {
  ...
  const handleReactivate = async (id: string, name: string) => {
    try {
      setDeleting(true)
      await dentalActsApi.reactivate(id)
   ...
   *
   * It is a bulk, irreversible write with no inverse endpoint — one click moved every provisional row in le catalogue des actes dentaires
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end">
                    ...
                    {a.isActive ? (
                      ...
                      <DropdownMenuItem
                        disabled={deleting}
                        onSelect={() => void handleReactivate(a.id, a.designationFr)}
                      >
                        Réactiver
                      </DropdownMenuItem>
                    )}
                        ...
                        <div className="flex justify-end gap-2">
                          ...
                          {act.isActive ? (
                            ...
                            <Button
                              ...
                              size="sm"
                              disabled={deleting}
                              onClick={() => void handleReactivate(act.id, act.designationFr)}
                              className="h-8 gap-1"
                            >
      ...
      <AlertDialog open={confirmDataOpen} onOpenChange={(open) => !open && setConfirmDataOpen(false)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Toutes les entrées encore marquées «&nbsp;à vérifier&nbsp;» dans le catalogue des actes dentaires seront confirmées d'un
