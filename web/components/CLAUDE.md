...
| `patient-record-modal.tsx` | **Tooth-first** dental-record entry (two panes): the sticky left `RecordToothChart` is a *selection* surface (+ Haut/Bas/Toute-la-bouche shortcuts, « à traiter » quick-select) overlaid with the patient's prior odontogram state; the right pane composes acts against that selection and lists the session grouped by tooth. Derives `isAdultTeeth` on save (mixed dentition allowed), mirrors « montant payé » to the running total until edited, and keeps the optional plan-item link (prefills the composer). |
| `record/use-session-acts.ts` | The record modal's `useReducer` store: charted `acts`, chart `selection`, the composed `draft`, and `editingKey`. All prefilling (edit mode, catalog pick, plan-step link) is an explicit dispatch, so no effect can race user input. Exports `resolveActCost` (unit × teeth, or flat) + `hasInvalidPrice`. **Never infers per-tooth pricing when loading a saved act** — it reads `unitCost`/`isPerTooth` provenance, so re-saving is cost-neutral. |
| `record/session-act-composer.tsx` | "What did you do to the selected teeth": catalog picker grouped by `pt.description` (the seed's de-facto category) with an « Autres » bucket, one editable price + `/dent ↔ forfait` toggle showing the billed total, MODVL faces, état résultant, note. Commits on `Entrée` (suppressed while the picker is open) and **keeps the selection** so a second act on the same tooth is one keystroke. |
